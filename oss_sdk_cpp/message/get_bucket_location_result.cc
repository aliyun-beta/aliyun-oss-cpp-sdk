...
const std::string& GetBucketLocationResult::GetLocation() const {
  return location_;
}

bool GetBucketLocationResult::DeserializeFromXml(const std::string& content) {
  TiXmlDocument doc;
  if (doc.Parse(content.c_str(), 0, TIXML_ENCODING_UTF8) == NULL)
    return false;

  TiXmlNode* root_node = doc.FirstChild("LocationConstraint");
  if (root_node == NULL)
    return false;
  location_ = GetXmlNodeText(root_node);
