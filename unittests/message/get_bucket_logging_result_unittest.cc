...
static const char* kXmlContent =
  ...
  "    <TargetPrefix>test18</TargetPrefix>\n"
  "  </LoggingEnabled>\n"
  "</BucketLoggingStatus>\n"
  "";

TEST(GetBucketLoggingResult, allinone) {
  GetBucketLoggingResult result;
  bool parse_ok = result.DeserializeFromXml(kXmlContent);
  EXPECT_TRUE(parse_ok);

  EXPECT_EQ("mybucket", result.GetTargetBucket());
  EXPECT_EQ("test18", result.GetTargetPrefix());
}

TEST(GetBucketLoggingResult, ParseFailed) {
  GetBucketLoggingResult result;
  bool parse_ok = true;
