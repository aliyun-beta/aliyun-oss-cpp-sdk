...
TEST(StringPieceTest, CheckSTL) {
  ...
  ASSERT_EQ(a.rfind(d),
            static_cast<size_t>(a.as_string().rfind(std::string())));
...
TEST(StringPieceTest, CheckNULL) {
  ...
  StringPiece s(NULL);
  ASSERT_EQ(s.data(), reinterpret_cast<const char*>(NULL));
  ASSERT_EQ(s.size(), 0U);

  s.set(reinterpret_cast<const char*>(NULL));
  ASSERT_EQ(s.data(), reinterpret_cast<const char*>(NULL));
  ASSERT_EQ(s.size(), 0U);
}
...
TEST(StringPieceTest, CheckComparisons2) {
  ...
  ASSERT_EQ(abc.compare(StringPiece("abcdefghijklmnopqrstuvwxyz")), 0);

  ASSERT_TRUE(abc < StringPiece("abcdefghijklmnopqrstuvwxzz"));
  ASSERT_LT(abc.compare(StringPiece("abcdefghijklmnopqrstuvwxzz")), 0);

  ASSERT_TRUE(abc > StringPiece("abcdefghijklmnopqrstuvwxyy"));
  ASSERT_GT(abc.compare(StringPiece("abcdefghijklmnopqrstuvwxyy")), 0);
...
TEST(StringPieceTest, HeterogenousStringPieceEquals) {
  ...
  ASSERT_EQ("hello", StringPiece("hello"));
}
